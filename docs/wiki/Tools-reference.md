...
| `move_email` | `message_ids[], to_mailbox, to_account?, account?, mailbox?, dry_run=false, max_moves?` | batch default 1; undoable. `to_account` is the **destination** account — set it to move across accounts (without it a cross-account destination fails "target mailbox not found"); `account`/`mailbox` are **source** hints, not the destination. Nested destination mailboxes resolve too. |
