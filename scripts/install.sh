#!/usr/bin/env bash
# cobos-apple-mail-mcp — one-shot installer for macOS.
#
# Installs the CLI (uv / pipx / pip, whichever is available), writes the config,
# offers to build the local index, and registers the server with Claude Desktop
# (and its Cowork workspace) by merging one entry into claude_desktop_config.json.
#
# The registration step edits a file you may already have other MCP servers in,
# so it is deliberately conservative: it parses the existing JSON, aborts
# without writing if that JSON is malformed (rather than clobbering it), backs
# the file up first, and only touches the single "apple-mail" key — every other
# server you configured is preserved byte-for-byte.
#
...
Usage: bash scripts/install.sh [options]
  ...
  --no-client          Skip Claude Desktop / Cowork registration (install only)
  -y, --yes            Non-interactive: assume "yes" to every prompt
  -h, --help           Show this help

The script never sends, moves, or deletes any mail. It installs the CLI, writes
~/.cobos-apple-mail-mcp/config.toml, offers to build the local search index
(skip with --no-index), and adds a stdio server entry to Claude Desktop's config.
EOF
}
...
[ "$(uname -s)" = "Darwin" ] || die "this installer is macOS-only (Apple Mail lives here)."
ok "macOS detected"
...
# Resolve the installed executable to an ABSOLUTE path. GUI-launched clients
# (Claude Desktop) don't inherit a login shell's PATH, so we register the full
# path rather than the bare command name — the #1 "server won't start" gotcha.
BIN=""
for cand in \
  "apple-mail-mcp" \
...
[ -n "$BIN" ] || die "installed, but couldn't find the apple-mail-mcp executable. Add its bin dir (usually ~/.local/bin) to PATH and re-run, or register it manually."
ok "installed: $BIN"
...
# ---- 3. permissions ------------------------------------------------------
step "macOS permissions (required before indexing / writing)"
info "Full Disk Access — for the app that RUNS the server (your terminal now, and"
info "  Claude Desktop later): lets it read ~/Library/Mail to build the index."
...
register_claude_desktop() {
  ...
  if [ ! -e "/Applications/Claude.app" ] && [ ! -e "$cfg" ]; then
    ...
    if ! confirm "Write the config anyway?" n; then
      info "skipped Claude Desktop registration."
      return 0
    fi
  fi

  # The merge is done in Python: it preserves every other server, backs up the
  # file, and refuses to write over malformed JSON instead of destroying it.
  # Run inside `if` so `set -e` is suspended and a non-zero exit (malformed
  # JSON) becomes a warning rather than killing the whole installer.
...
PY
  then
    ok "registered — quit Claude Desktop completely (Cmd-Q) and reopen it to load the server."
  else
...
if [ "$DO_CLIENT" = 1 ]; then
  ...
  # Claude Code (the `claude` CLI) also powers Cowork automations; register there
  # too if it's installed, at user scope so every project/session sees it.
  # Two explicit forms rather than an array — macOS ships bash 3.2, where
  # expanding an empty array under `set -u` aborts with "unbound variable".
  if command -v claude >/dev/null 2>&1; then
    if confirm "Also register with the Claude Code CLI (user scope)?" y; then
      if [ "$READ_ONLY" = 1 ]; then
        claude_rc=0; claude mcp add --scope user "$SERVER_NAME" -- "$BIN" serve --read-only || claude_rc=$?
      else
        claude_rc=0; claude mcp add --scope user "$SERVER_NAME" -- "$BIN" serve || claude_rc=$?
      fi
      if [ "$claude_rc" = 0 ]; then
        ok "claude mcp: added \"$SERVER_NAME\""
      else
        warn "claude mcp add failed (maybe already registered) — check: claude mcp list"
      fi
    fi
  fi
fi
...
step "Done"
info "Next:"
info "  1. Grant Full Disk Access to Claude Desktop (same pane as above) if you haven't."
info "  2. Fully quit and reopen Claude Desktop."
